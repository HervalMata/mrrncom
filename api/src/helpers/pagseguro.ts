...
pagseguro.prototype.addItem = function (item) {
    ...
    this.checkoutData['itemId' + (this.items.length)] = item.items.length;
...
pagseguro.prototype.sendTransaction = function (transaction, cb) {
    ...
    this.checkoutData.installmentValue = (transaction.value / transaction.installments);
...
pagseguro.prototype.sessionId = function (cb) {
    const url = this.url + '/transactions?token' + this.token + '&email' + this.email;
